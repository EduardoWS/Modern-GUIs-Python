...
class App(ctk.CTk):
    def __init__(self):
        ...
        # tracing
        self.height_int.trace('w', self.update_imc)
        self.weight_float.trace('w', self.update_imc)
        self.metric_bool.trace('w', self.change_units)
        
        # widgets
        ResultText(self, self.imc_string)
    ...
    def update_imc(self, *args):
        ...
        imc_result = round(weight_kg / height_meter ** 2, 1)
        self.imc_string.set(imc_result)

class ResultText(ctk.CTkLabel):
    def __init__(self, parent, imc_string):
        font = ctk.CTkFont(family=FONT, size=MAIN_TEXT_SIZE, weight='bold')
        super().__init__(master=parent, text=22.5, font=font, text_color=WHITE, textvariable=imc_string)
...
class WeightInput(ctk.CTkFrame):
    def __init__(self, parent, weight_float, metric_bool):
        ...
        self.output_string.set(str(f'{weight_float.get()}kg'))
    ...
    def update_weight(self, info=None):
        ...
        if self.metric_bool.get():
            self.output_string.set(str(f'{round(self.weight_float.get(),1)}kg'))
        else:
            raw_ounces = self.weight_float.get() * 2.205 * 16
            pounds, ounces = divmod(raw_ounces, 16)
            self.output_string.set(str(f'{int(pounds)}lb {int(ounces)}oz'))
...
class HeightInput(ctk.CTkFrame):
    def __init__(self, parent, height_int, metric_bool):
        ...
        output_text = ctk.CTkLabel(master=self, textvariable=self.output_string, text_color=BLACK, font=ctk.CTkFont(family=FONT, size=INPUT_FONT_SIZE))
